...
const Cards = ({search, selectedFilter, opitonList} : CardsProps) => {
  const [list, setList] = useState(dataMenu); // estou passanddo o objeto aqui de dataCards e chamando o primeiro state de list no MAP
  ...
  useEffect(() => {
    const newList = dataMenu.filter((item) => testSearch(item.title) && testTag(item.category.id));
